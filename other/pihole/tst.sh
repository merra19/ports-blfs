...
package_manager_detect() {
    ...
    if is_command scratch ; then
        ...
        UPDATE_PKG_CACHE="${PKG_MANAGER} sync"
        # The command we will use to actually install packages
        PKG_INSTALL=("${PKG_MANAGER}" install -y)
        # grep -c will return 1 if there are no matches. This is an acceptable condition, so we OR TRUE to prevent set -e exiting the script.
        PKG_COUNT="${PKG_MANAGER} outdate | grep -q '==>' && true || false"
...
installConfigs() {
    ...
    if [[ -d '/run/systemd/system' ]]; then
        ...
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc0.d/K27pihole-FT
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc1.d/K27pihole-FT
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc2.d/S33pihole-FT
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc3.d/S33pihole-FT
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc4.d/S33pihole-FT
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc5.d/S33pihole-FT
        ln -sf  ../init.d/pihole-FT /etc/rc.d/rc6.d/K27pihole-FT
    fi
...
stop_service() {
    ...
    if is_command systemctl ; then
        systemctl stop "${1}" &> /dev/null || true
         ...
         [ -f /etc/init.d/"${1}" ] && /etc/init.d/"${1}" stop &> /dev/null || true
    fi
    printf "%b  %b %s...\\n" "${OVER}" "${TICK}" "${str}"
}
...
# Enable service so that it will start with n
    # Local, named variablesext reboot
enable_service() {
    ...
    if is_command systemctl ; then
        ...
        case $1 in
            lighttpd)
                ...
                ;;
            pihole-FTL)
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc0.d/K27pihole-FTL
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc1.d/K28pihole-FTL
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc2.d/S33pihole-FTL
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc3.d/S33pihole-FTL
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc4.d/S33pihole-FTL
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc5.d/S33pihole-FTL
                ln -sf  ../init.d/pihole-FTL /etc/rc.d/rc6.d/K27pihole-FTL
                ;;
        esac
    fi
    printf "%b  %b %s...\\n" "${OVER}" "${TICK}" "${str}"
...
notify_package_updates_available() {
    ...
    # $(uname -r)
    if [[ -d "/lib/modules/6.12.8-LFS" ]]; then
...
install_dependent_packages() {
    ...
    for i in "$@"; do
            ...
            if scratch isinstalled "${i}" 2>/dev/null ; then
